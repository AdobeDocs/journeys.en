...
For external data sources, the maximum number of calls per second is set to 15. If the number of calls exceeds 15 per second, the remaining calls are discarded. You can increase this limit for private external data sources. Contact Adobe to include the endpoint in the allowlist. This is not possible for public external data sources. To learn more on best practices and guardails when integrating external systems, refer to this [page](../about/external-systems.md).
