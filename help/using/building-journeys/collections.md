...
This section shows the generic procedure 

For this section, we will use the following JSON payload example. This is an array of objects with a field that is a simple collection.
...
You can see that "products" is an array of two objets. Only one object is required.
...
1. If needed, adjust the field types if needed. The following field types are supported for collections: listString, listInteger, listDecimal, listBoolean, listDateTime, listDateTimeOnly, listDateOnly, listObject

   >[!NOTE]
   >
   >The field type is automatically inferred according to the payload example.

1. If you want to pass objects dynamically, you need to set them as variables. In this example we set "products" as variables. All the object fields included in the object are set to variables automatically.
...
1. In the Action parameters, define the array parameter ("products" in our example) using the advanced expression editor. [Learn more](../building-journeys/using-custom-actions.md).

1. For each of the following object field, type the corresponding field name from the source XDM schema. If the names are identical, this is not needed. In our example, we only need to define "product id".
